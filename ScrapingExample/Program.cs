...
namespace ScrapingExample
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
           ...
           var Locations = new string[] {"losangeles", "San Diego", "orangecounty", "sfbay" };

            for (var i = 0; i < Locations.Length; i++)
            {

                for (var listing = 0; listing < 100; listing += 120)
                {
                    ...
                    var doc = context.OpenAsync("https://" + Locations[i] + ".craigslist.org/search/sga?s=" + listing + "&query=surfboard").Result;
                    ...
                    using (var con = new SqlConnection(ConfigurationManager.ConnectionStrings["SurfboardDBConnection"].ConnectionString))
                    {
                        ...
                        foreach (var node in nodes)
                        {
                            ...
                            surfboard.Location = Locations[i];
                            ...
                            cmd.CommandType = CommandType.StoredProcedure;
                            {
                                ...
                                cmd.Parameters.AddWithValue("@Location", surfboard.Location);
